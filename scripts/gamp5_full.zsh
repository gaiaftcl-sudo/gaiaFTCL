...
verbs:
  ...
  - id: LG-FRANKLIN-EMBODY-ORB-BREATHE-001
    category: embody
    display_verb_phrase: "Orb resting breath"
    interaction_class: C
    witness_primitive: franklin_ui_event
...
test_suites:
  ...
  FranklinPresenceTests:
    surface: FranklinApp
    narration: "Franklin presence layer: orb, facets, canvas, tray. The orb breathes on the closure-window heartbeat; facets are bound to cell_manifest_registry.yaml and never hardcoded."
    catalog_verbs: [LG-FRANKLIN-EMBODY-ORB-BREATHE-001]
  LithographyTests:
    surface: Lithography
...
# Visible orb (canonical user-visible Franklin).
FRANKLIN_ORB_DIR="${FRANKLIN_ROOT}/GAIAOS/macos/Franklin"
...
if [[ ! -d "${FRANKLIN_ORB_DIR}" ]]; then
    REFUSE_TERMINAL GW_REFUSE_FRANKLIN_NOT_LAUNCHED \
        LG-FRANKLIN-IQ-LAUNCH-FRANKLIN-001 \
        "FranklinApp orb source tree missing at ${FRANKLIN_ORB_DIR}; nothing to launch."
fi
...
# Build the orb via SwiftPM (release). No synthetic .app fabrication.
if [[ ! -x "${FRANKLIN_ORB_BIN}" ]]; then
    ( cd "${FRANKLIN_ORB_DIR}" && swift build -c release --product FranklinApp ) \
        >"${FRANKLIN_ORB_BUILD_LOG}" 2>&1 || \
        REFUSE_TERMINAL GW_REFUSE_FRANKLIN_NOT_LAUNCHED \
            LG-FRANKLIN-IQ-LAUNCH-FRANKLIN-001 \
            "FranklinApp orb swift build failed; see ${FRANKLIN_ORB_BUILD_LOG}"
fi
[[ -x "${FRANKLIN_ORB_BIN}" ]] || REFUSE_TERMINAL GW_REFUSE_FRANKLIN_NOT_LAUNCHED \
    LG-FRANKLIN-IQ-LAUNCH-FRANKLIN-001 \
    "FranklinApp orb binary missing at ${FRANKLIN_ORB_BIN}"
...
# The orb needs to know which envelope to consume; we resolve and export it
...
# and let the handoff step overwrite it. Pre-writing keeps the orb's
...
# Launch the bridge first (no UI side effects). The orb is launched after
...
# The orb is launched in LG-FRANKLIN-IQ-HANDOFF-001 below, AFTER the
# envelope is fully populated. The launch witness here records that
# the bridge came up and the orb binary is ready; the orb pid + window
...
WITNESS LG-FRANKLIN-IQ-LAUNCH-FRANKLIN-001 CALORIE \
    "bridge_pid=${FRANKLIN_BRIDGE_PID} :8830 live; orb binary ready at ${FRANKLIN_ORB_BIN}"
...
# ----------------------------------------------------------------------------
# IQ — visible orb launch. The envelope is now fully populated; launch the
# orb (FranklinApp from GAIAOS/macos/Franklin/.build/release/FranklinApp)
# so it reads the envelope on onAppear and starts the in-process OQ
# catalog runner. The orb is the user-visible deliverable: a SwiftUI
# WindowGroup with a glass orb the operator can see and tap. The proof
# is pid+visible-window, not menu-bar — this is a regular foreground app.
# ----------------------------------------------------------------------------

NARRATE LG-FRANKLIN-IQ-FRANKLIN-VISIBLE-001 B \
"Launching the visible Franklin orb with the run envelope. The orb's \
...
operator must see the orb."

# Use `open` so the orb gets a normal foreground app activation. We pass
# the envelope path through the inherited environment.
FRANKLIN_ORB_LAUNCH_LOG="${EVIDENCE_ROOT}/franklin_orb_launch.log"
FRANKLIN_HANDOFF_ENVELOPE="${FRANKLIN_HANDOFF_ENVELOPE}" \
FRANKLIN_ROOT="${FRANKLIN_ROOT}" \
    "${FRANKLIN_ORB_BIN}" \
        >"${EVIDENCE_ROOT}/franklin_orb_stdout.log" \
        2>"${EVIDENCE_ROOT}/franklin_orb_stderr.log" &
FRANKLIN_ORB_LAUNCH_RC=$?
sleep 1.0

FRANKLIN_ORB_PID="$(pgrep -x FranklinApp | sed -n '1p')"
if [[ -z "${FRANKLIN_ORB_PID}" ]]; then
    REFUSE_TERMINAL GW_REFUSE_FRANKLIN_NOT_LAUNCHED \
        LG-FRANKLIN-IQ-FRANKLIN-VISIBLE-001 \
        "FranklinApp orb did not start; rc=${FRANKLIN_ORB_LAUNCH_RC}; see franklin_orb_stderr.log"
    return 1
fi
...
cat > "${EVIDENCE_ROOT}/franklin_orb_launch.json" <<JSON
{
  ...
  "orb_binary": "${FRANKLIN_ORB_BIN}",
  "orb_pid": ${FRANKLIN_ORB_PID},
  "window_verified": ${FRANKLIN_ORB_WINDOW_VERIFIED},
  "envelope_path": "${FRANKLIN_HANDOFF_ENVELOPE}",
  "tau": "${TAU_HUMAN}"
}
JSON
WITNESS LG-FRANKLIN-IQ-FRANKLIN-VISIBLE-001 CALORIE \
    "orb_pid=${FRANKLIN_ORB_PID} window_verified=${FRANKLIN_ORB_WINDOW_VERIFIED} envelope=${FRANKLIN_HANDOFF_ENVELOPE}"
...
# OQ catalog execution is delegated to the live orb process. The orb
...
"Waiting for the live orb to complete the OQ catalog. The orb is running \
...
timeout (FRANKLIN_OQ_TIMEOUT_SECONDS, default 900s). Timeout = the orb \
...
while [[ ! -s "${FRANKLIN_HANDOFF_COMPLETE}" ]]; do
    if (( oq_waited >= FRANKLIN_OQ_TIMEOUT_SECONDS )); then
        REFUSE_TERMINAL GW_REFUSE_OQ_CATALOG_INCOMPLETE \
            ...
            "orb did not write handoff_complete.json within ${FRANKLIN_OQ_TIMEOUT_SECONDS}s; expected at ${FRANKLIN_HANDOFF_COMPLETE}"
        break
    fi
    sleep "${FRANKLIN_OQ_POLL_INTERVAL}"
    oq_waited=$((oq_waited + FRANKLIN_OQ_POLL_INTERVAL))
    # Sanity: if the orb died, do not wait the full timeout.
    if ! kill -0 "${FRANKLIN_ORB_PID}" 2>/dev/null; then
        REFUSE_TERMINAL GW_REFUSE_FRANKLIN_NOT_LAUNCHED \
            LG-FRANKLIN-OQ-LIVE-CATALOG-001 \
            "orb pid ${FRANKLIN_ORB_PID} died mid-catalog after ${oq_waited}s; see franklin_orb_stderr.log"
        break
    fi
done
...
if [[ -s "${FRANKLIN_HANDOFF_COMPLETE}" ]]; then
    ...
    WITNESS LG-FRANKLIN-OQ-LIVE-CATALOG-001 CALORIE \
        "orb closed handoff: total=${oq_total:-?} calorie=${oq_calorie:-?} refused=${oq_refused:-?} (waited ${oq_waited}s)"
fi
...
"PQ is orb-owned. Runner waits for evidence/runs/<tau>/pq_complete.json \
from the live orb process and then proceeds to Reconcile/Epilogue."
...
while [[ ! -s "${FRANKLIN_PQ_COMPLETE}" ]]; do
    if (( pq_waited >= FRANKLIN_PQ_TIMEOUT_SECONDS )); then
        REFUSE_TERMINAL GW_REFUSE_OQ_CATALOG_INCOMPLETE \
            ...
            "orb did not write pq_complete.json within ${FRANKLIN_PQ_TIMEOUT_SECONDS}s; expected at ${FRANKLIN_PQ_COMPLETE}"
        break
    fi
    ...
    if ! kill -0 "${FRANKLIN_ORB_PID}" 2>/dev/null; then
        REFUSE_TERMINAL GW_REFUSE_FRANKLIN_NOT_LAUNCHED \
            LG-FRANKLIN-PQ-EDU-AUDITOR-001 \
            "orb pid ${FRANKLIN_ORB_PID} died mid-PQ after ${pq_waited}s; see franklin_orb_stderr.log"
        break
    fi
done
if [[ -s "${FRANKLIN_PQ_COMPLETE}" ]]; then
    ...
    WITNESS LG-FRANKLIN-PQ-EDU-AUDITOR-001 CALORIE \
        "orb closed PQ handoff: calorie=${pq_calorie:-?} refused=${pq_refused:-?} (waited ${pq_waited}s)"
fi
