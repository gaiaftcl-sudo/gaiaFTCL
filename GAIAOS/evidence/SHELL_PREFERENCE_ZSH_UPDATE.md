...
**File:** `.cursor/rules/shell-preference-zsh.mdc` (mirrored to `GAIAOS/.cursor/rules/`)
...
#### `GAIAOS/.cursor/rules/cell-operator-autonomous-closure.mdc`
...
#### `GAIAOS/.cursor/rules/uum8d-mooring-progress.mdc`
- Line 27: `bash -n` → `zsh -n` (parallel verification)

#### `GAIAOS/.cursor/rules/prod-push-complete.mdc`
- Line 16: `bash scripts/mesh_health_snapshot.sh` → `zsh scripts/mesh_health_snapshot.sh`

#### `GAIAOS/.cursor/rules/playwright-gaiaftcl.mdc`
...
- Line 14: `bash scripts/run_operator_fusion_mesh_closure.sh` → `zsh scripts/run_operator_fusion_mesh_closure.sh`
- Line 16: `OPERATOR_CLOSURE_SKIP_WORKING_APP=1 bash` → `OPERATOR_CLOSURE_SKIP_WORKING_APP=1 zsh`
- Line 18: `bash GAIAOS/scripts/run_full_release_session.sh` → `zsh GAIAOS/scripts/run_full_release_session.sh`
...
**Changes:**
1. Line 15: `bash GAIAOS/scripts/gaiafusion_kernel_purge.sh` → `zsh GAIAOS/scripts/gaiafusion_kernel_purge.sh`
...
- This receipt: `GAIAOS/evidence/SHELL_PREFERENCE_ZSH_UPDATE.md`
