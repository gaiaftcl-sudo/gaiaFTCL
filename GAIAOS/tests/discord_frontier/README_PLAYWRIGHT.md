...
```bash
bash GAIAOS/scripts/run_discord_membrane_playwright.sh
```

Or:

```bash
cd GAIAOS/tests/discord_frontier/playwright && npm install && npx playwright install chromium && npx playwright test
```
...
1. On a machine with a browser: generate storage once (do not commit the file):
   ...
   ```bash
   cd GAIAOS/tests/discord_frontier/playwright
...
2. Run automation (opens **Information**, **Bot**, **OAuth2 URL generator**, toggles **bot** + **applications.commands** when checkboxes exist, writes screenshots + JSON under `evidence/discord_closure/dev_portal/`):
   ...
   bash GAIAOS/scripts/run_discord_dev_portal_playwright.sh
   ```
...
`INTEGRATION_DISCORD_PLAYWRIGHT=1 bash GAIAOS/scripts/validate_discord_game_rooms.sh` runs registry checks plus Playwright.
