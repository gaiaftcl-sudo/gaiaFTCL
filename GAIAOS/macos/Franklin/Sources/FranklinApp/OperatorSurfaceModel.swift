...
struct PresenceEvidencePayload: Codable {
    ...
    let avatar_state_at_invocation: AvatarStateAtInvocationPayload
...
struct AvatarStateAtInvocationPayload: Codable {
    let posture: String
...
struct AvatarLanguageGame: Identifiable {
    let id: String
    let title: String
    let scope: String
    let executable: Bool
...
final class OperatorSurfaceModel: ObservableObject {
    ...
    @Published var avatarAudioInputEnabled: Bool = false
    @Published var avatarVisualFocusEnabled: Bool = true
    @Published var avatarRecordingEnabled: Bool = false
    @Published var latestRecordingReceiptPath: String = ""
    ...
    func dispatchRoute(modality: InvocationModality = .type, overridePrompt: String? = nil) async {
        let sourcePrompt = overridePrompt ?? routePrompt
    ...
    func avatarGreetAndGuide() {
        appendConversation(
            speaker: "Franklin",
            facet: activeFacet,
            message: "Welcome. I am your avatar control surface. I can route chat, voice, visual checks, and recording while guiding all language games available for \(activeFacet.rawValue)."
        )
        FranklinSpeechLoopService.shared.speak("Welcome. I stand ready to guide your language games.")
        let games = currentFacetLanguageGames()
            .map { "\($0.id) (\($0.scope)) — \($0.title)" }
            .joined(separator: "\n")
        appendConversation(
            speaker: "Franklin",
            facet: activeFacet,
            message: "Available language games for \(activeFacet.rawValue):\n\(games)"
        )
    }

    func toggleAudioInput() {
        avatarAudioInputEnabled.toggle()
        if avatarAudioInputEnabled {
            FranklinSpeechLoopService.shared.startListening()
        } else {
            FranklinSpeechLoopService.shared.stopListening()
        }
        appendConversation(
            speaker: "Franklin",
            facet: activeFacet,
            message: "Audio input \(avatarAudioInputEnabled ? "enabled" : "disabled"). Voice routes \(avatarAudioInputEnabled ? "are ready" : "are paused")."
        )
    }

    func toggleVisualFocus() {
        avatarVisualFocusEnabled.toggle()
        if avatarVisualFocusEnabled {
            FranklinVisionAttentionService.shared.start()
        } else {
            FranklinVisionAttentionService.shared.stop()
        }
        appendConversation(
            speaker: "Franklin",
            facet: activeFacet,
            message: "Visual pipeline \(avatarVisualFocusEnabled ? "enabled" : "disabled")."
        )
    }

    func toggleRecording() {
        if avatarRecordingEnabled {
            do {
                if let receipt = try FranklinRecordService.shared.stop() {
                    latestRecordingReceiptPath = receipt.path
                }
                avatarRecordingEnabled = false
                appendConversation(
                    speaker: "Franklin",
                    facet: activeFacet,
                    message: "Recording stopped. Receipt path: \(latestRecordingReceiptPath.isEmpty ? "unavailable" : latestRecordingReceiptPath)"
                )
            } catch {
                appendConversation(
                    speaker: "Franklin",
                    facet: activeFacet,
                    message: "REFUSED: failed to stop recording (\(error.localizedDescription))."
                )
                lastResult = "REFUSED: recording stop failed"
            }
            return
        }
        do {
            let tau = ProcessInfo.processInfo.environment["FOT_SPROUT_TAU"] ?? ISO8601DateFormatter().string(from: Date())
            let root = resolveEvidenceRootURL()
                ?? FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
                ?? URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            let out = root
                .appendingPathComponent("recordings", isDirectory: true)
                .appendingPathComponent("\(tau).receipt.json")
            try FranklinRecordService.shared.start(
                FranklinRecordRequest(
                    outputURL: out,
                    tau: tau,
                    lgID: "LG-FRANKLIN-PQ-AVATAR-LIFELIKE-001"
                )
            )
            avatarRecordingEnabled = true
            appendConversation(
                speaker: "Franklin",
                facet: activeFacet,
                message: "Recording started. I will write receipt evidence when stopped."
            )
        } catch {
            appendConversation(
                speaker: "Franklin",
                facet: activeFacet,
                message: "REFUSED: failed to start recording (\(error.localizedDescription))."
            )
            lastResult = "REFUSED: recording start failed"
        }
    }

    func runLanguageGame(_ gameID: String) async {
        guard currentFacetLanguageGames().contains(where: { $0.id == gameID && $0.executable }) else {
            appendConversation(
                speaker: "Franklin",
                facet: activeFacet,
                message: "REFUSED: language game \(gameID) is not executable from this avatar surface yet."
            )
            lastResult = "REFUSED: non-executable language game"
            return
        }
        let prompt = "run language game \(gameID)"
        routePrompt = prompt
        await dispatchRoute(modality: avatarAudioInputEnabled ? .voice : .chipTap, overridePrompt: prompt)
    }

    func currentFacetLanguageGames() -> [AvatarLanguageGame] {
        (FranklinLanguageGameCatalog.byFacet[activeFacet] ?? []) + FranklinLanguageGameCatalog.shared
    }

    var avatarIsListening: Bool {
        avatarAudioInputEnabled
    }

    var avatarIsRecording: Bool {
        avatarRecordingEnabled
    }

    var avatarIsSpeaking: Bool {
        guard let last = conversationColumn.last else { return false }
        guard last.speaker == "Franklin" else { return false }
        return Date().timeIntervalSince(last.timestamp) < 4.0
    }
    ...
    func makeDispatchPayload(
        ...
        return FranklinDispatchPayload(
            ...
            presence_evidence: PresenceEvidencePayload(
                ...
                avatar_state_at_invocation: AvatarStateAtInvocationPayload(
                    posture: "calm_breath",
                    color_cast: "green",
