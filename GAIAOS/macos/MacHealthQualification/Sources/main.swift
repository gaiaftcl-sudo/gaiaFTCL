// MacHealth IQ/OQ/PQ Qualification
...
func runIQ(appPath: String) -> String {
    banner("MacHealth — IQ (Installation Qualification)")
    let fm = FileManager.default
    ...
    // Check staticlibs
    let libs = ["libgaia_health_renderer.a", "libbiologit_md_engine.a", "libbiologit_usd_parser.a"]
    for lib in libs {
        guard fm.fileExists(atPath: "\(appPath)/MetalRenderer/lib/\(lib)") else {
            fail("\(lib) missing")
        }
    }
    ok("Staticlibs present (3)")
    
    // Check header
    guard fm.fileExists(atPath: "\(appPath)/MetalRenderer/include/gaia_health_renderer.h") else {
        fail("gaia_health_renderer.h missing")
    }
    ok("Header present")
    
    // Build
    print("  Building MacHealth...")
    let (exitCode, output) = runProcess(["swift", "build", "--product", "MacHealth"], workingDir: appPath)
    if exitCode != 0 {
    ...
    // Check executable
    guard fm.fileExists(atPath: "\(appPath)/.build/debug/MacHealth") else {
        fail("Executable missing")
    }
    ok("Executable: .build/debug/MacHealth")
    
    // Write receipt
    let receipt = Receipt(
        spec: "MACHEALTH-IQ-001",
        phase: "IQ",
        cell: "MacHealth",
    ...
    writeReceipt(receipt, path: "\(appPath)/evidence/iq/machealth_iq_receipt.json")
...
func runOQ(appPath: String) -> String {
    banner("MacHealth — OQ (Operational Qualification)")
    
    let filters = [
        "testRendererCreateDestroy",
        "testEpistemicRoundTrip",
        "testFrameCountIncrements",
        "testNullHandleSafety",
        "testOutOfRangeEpistemicClamped"
    ]
    ...
    writeReceipt(receipt, path: "\(appPath)/evidence/oq/machealth_oq_receipt.json")
...
func runPQ(appPath: String) -> (String, String) {
    banner("MacHealth — PQ (Performance Qualification)")
    ...
    writeReceipt(receipt, path: "\(appPath)/evidence/pq/machealth_pq_receipt.json")
...
func main() {
    let appPath = URL(fileURLWithPath: #file)
        ...
        .deletingLastPathComponent()
        .deletingLastPathComponent()
        .appendingPathComponent("MacHealth")
        .path
    
    banner("MacHealth Qualification — IQ → OQ → PQ")
    
    let iq = runIQ(appPath: appPath)
    let oq = runOQ(appPath: appPath)
    let (pq, gpu) = runPQ(appPath: appPath)
    
    banner("MacHealth Qualification Complete")
