...
for cell_ip in "${ALL_CELLS[@]}"; do
    ...
    # Copy service files
    ssh -i $SSH_KEY root@$cell_ip "mkdir -p /root/GAIAOS/services/wallet_observer"
    scp -i $SSH_KEY services/wallet_observer/* root@$cell_ip:/root/GAIAOS/services/wallet_observer/
    
    # Build and run on cell
    ssh -i $SSH_KEY root@$cell_ip << 'ENDSSH'
cd /root/GAIAOS/services/wallet_observer
docker build -t gaiaftcl/wallet_observer:latest .
