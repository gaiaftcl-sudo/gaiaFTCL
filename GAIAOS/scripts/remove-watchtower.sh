...
run_cmd '
    ...
    for f in /root/GAIAOS/docker-compose*.yml /root/docker-compose*.yml; do
        if [ -f "$f" ]; then
...
run_cmd '
    mkdir -p /root/GAIAOS/ftcl/compliance
    
    cat > /root/GAIAOS/ftcl/compliance/UPDATE_PROTOCOL_COMPLIANCE.json << EOF
{
