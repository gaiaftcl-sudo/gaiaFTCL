...
export default async function BiospherePage() {
  const [tiles, obsWildfire, obsModis] = await Promise.all([
    fetchRecentTiles("biosphere_tiles", 200, "valid_time"),
    fetchRecentObservations(100, "biosphere_wildfire"),
    fetchRecentObservations(100, "biosphere_modis_ndvi_catalog"),
  ]);

  return (
    <main className="mx-auto max-w-6xl px-4 py-6">
      <div className="mb-4">
        <div className="text-xl font-semibold text-white">Biosphere</div>
        <div className="text-sm text-zinc-400">Tiles: biosphere_tiles, Observations: wildfires + MODIS catalog</div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
            ...
            {tiles.tiles.slice(0, 25).map((t: any) => (
              <div key={t._key} className="rounded-md bg-zinc-900 px-3 py-2">
                <div className="text-xs text-zinc-200">
                  ...
                  <span className="text-zinc-500">{t.state?.wildfire_event_count ?? ""}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
          ...
          <div className="mt-3 space-y-2">
            {[...obsWildfire.observations.slice(0, 12), ...obsModis.observations.slice(0, 13)].map((o: any) => (
