//! QFOT Field Validator
//!
//! Validates field-substrate prediction tiles against closure gates:
//! - Temporal closure: forecast_time < valid_time
//! - Observer closure: predictions do not masquerade as observations
//! - Provenance: prediction source and model version are present
//!
//! This module writes audit artifacts into `field_validations` (ArangoDB `gaiaos`).
...
pub struct QfotFieldValidationRequest {
    pub target_collection: String,
    pub keys: Vec<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct QfotFieldValidationResponse {
    pub passed: bool,
    pub validated: usize,
    pub failed: usize,
...
impl Arango {
    fn new() -> Result<Self> {
        ...
        let http = reqwest::Client::builder()
            ...
            .user_agent("GaiaOS-Validation-QFOT/0.1.0")
            .build()
...
pub async fn validate_predictions(req: QfotFieldValidationRequest) -> Result<QfotFieldValidationResponse> {
    let arango = Arango::new()?;

    if req.keys.is_empty() {
        return Ok(QfotFieldValidationResponse {
            passed: false,
            validated: 0,
            failed: 1,
    ...
    for key in &req.keys {
        let doc = arango.get_doc(&req.target_collection, key).await?;
        validated += 1;
        ...
        if let Some(arr) = doc["observations"].as_array() {
            if !arr.is_empty() {
                failures.push(format!("{key}: predictions_must_not_include_observations"));
            }
        }
    }
    ...
    // Persist audit record (append-only evidence)
    let record = json!({
        ...
        "failures": failures,
        "provenance": {
            "source": "gaiaos_validation::qfot_field",
    ...
    Ok(QfotFieldValidationResponse {
        passed,
        validated,
        failed,
