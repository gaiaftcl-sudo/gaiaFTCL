//! QFOT Molecular Validator
//!
//! Fail-closed validation gates for molecular prediction tiles.
...
//! - observer closure (predictions must not claim experimental observations)
...
pub struct QfotMolecularValidationRequest {
    pub keys: Vec<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct QfotMolecularValidationResponse {
    pub passed: bool,
    pub validated: usize,
    pub failed: usize,
...
impl Arango {
    fn new() -> Result<Self> {
        ...
        let http = reqwest::Client::builder()
            ...
            .user_agent("GaiaOS-Validation-QFOT-Molecular/0.1.0")
            .build()
...
pub async fn validate_molecular_predictions(req: QfotMolecularValidationRequest) -> Result<QfotMolecularValidationResponse> {
    let arango = Arango::new()?;
    let target_collection = "molecular_tiles";

    if req.keys.is_empty() {
        return Ok(QfotMolecularValidationResponse {
            passed: false,
            validated: 0,
            failed: 1,
    ...
    for key in &req.keys {
        ...
        // Observer closure: predictions must not include experimental observations blob.
        if doc.get("observations").is_some() {
            failures.push(format!("{key}: predictions_must_not_include_observations_field"));
        }

        // Physical closure (minimal): energy fields must exist (no fabricated "validations" without core physics).
        let pe = doc["state"]["potential_energy"].as_f64();
        let ke = doc["state"]["kinetic_energy"].as_f64();
        if pe.is_none() || ke.is_none() {
            failures.push(format!("{key}: missing_energy_terms state.potential_energy/state.kinetic_energy"));
        }
    }
    ...
    let record = json!({
        ...
        "failures": failures,
        "provenance": {
            "source": "gaiaos_validation::qfot_molecular",
    ...
    Ok(QfotMolecularValidationResponse {
        passed,
        validated,
        failed,
