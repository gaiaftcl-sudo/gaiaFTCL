...
ssh -i "$SSH_KEY" -o ConnectTimeout=10 -o StrictHostKeyChecking=no "root@${CELL_IP}" \
  "cd /opt/gaiaftcl && mkdir -p /var/www/gaiaftcl/cells/fusion/evidence/{ui_expected,ui_contract,agent_census,domain_tubes,closure_game,echo,mcp_calls} && docker rm -f gaiaos-ui-tester-mcp 2>/dev/null || true && docker compose up -d gaiaos-ui-tester-mcp"
