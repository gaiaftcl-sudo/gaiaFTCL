...
set -uo pipefail
RSYNC_SRC="${RSYNC_SRC:-/Users/richardgillespie/Documents/FoT8D/cells/fusion/}"
EXCLUDES=(
...
run_remote_deploy() {
  ...
  if [ -n "${MESH_INTERNAL_KEY:-}" ]; then
    REMOTE="$(printf 'export GAIAFTCL_INTERNAL_KEY=%q CELL_ID=%q CELL_IP=%q && bash /opt/gaia/cells/fusion/scripts/crystal_remote_deploy.sh' "$MESH_INTERNAL_KEY" "$CELL" "$IP")"
  else
    REMOTE="$(printf 'export CELL_ID=%q CELL_IP=%q && bash /opt/gaia/cells/fusion/scripts/crystal_remote_deploy.sh' "$CELL" "$IP")"
  fi
...
for CELL in "${HETZNER[@]}"; do
  ...
  rsync -az --delete "${EXCLUDES[@]}" "$RSYNC_SRC" "root@${IP}:/opt/gaia/cells/fusion/" || { echo "RSYNC_FAILED $CELL"; echo -e "${CELL}\tNO\tNO\tNO" >>"$RESULT_FILE"; continue; }
  REMOTE_CMD=""
  if [ -n "${MESH_INTERNAL_KEY:-}" ]; then
    REMOTE_CMD="$(printf 'export GAIAFTCL_INTERNAL_KEY=%q CELL_ID=%q CELL_IP=%q && bash /opt/gaia/cells/fusion/scripts/crystal_remote_deploy.sh' "$MESH_INTERNAL_KEY" "$CELL" "$IP")"
  else
    REMOTE_CMD="$(printf 'export CELL_ID=%q CELL_IP=%q && bash /opt/gaia/cells/fusion/scripts/crystal_remote_deploy.sh' "$CELL" "$IP")"
  fi
