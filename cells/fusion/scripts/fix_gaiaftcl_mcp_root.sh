...
scp -i "$SSH_KEY" -o ConnectTimeout=10 -o StrictHostKeyChecking=no \
  "$cells/fusion/services/substrate_generative_sidecar/generative_api.py" \
  "$cells/fusion/services/substrate_generative_sidecar/Dockerfile" \
...
scp -i "$SSH_KEY" -o ConnectTimeout=10 -o StrictHostKeyChecking=no \
  "$cells/fusion/services/franklin_guardian/docker-compose.internal-life.yml" \
  root@$CELL_HOST:/root/gaiaos/services/franklin_guardian/
scp -i "$SSH_KEY" -o ConnectTimeout=10 -o StrictHostKeyChecking=no \
  "$cells/fusion/services/life_game/franklin_reflection_game.py" \
