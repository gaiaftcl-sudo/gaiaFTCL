...
for CELL in "${CELLS[@]}"; do
  ...
  if ssh -i "$SSH_KEY" -o ConnectTimeout=10 -o StrictHostKeyChecking=no "root@${IP}" bash <<'REMOTE_SCRIPT'
    ...
    mkdir -p /var/www/gaiaftcl/cells/fusion/evidence/{ui_expected,ui_contract,agent_census/canon,domain_tubes,closure_game,echo,mcp_calls}
    ...
    docker run -d \
      ...
      --network host \
      -v /var/www/gaiaftcl/cells/fusion/evidence:/app/evidence:rw \
      -e RUST_LOG=info \
