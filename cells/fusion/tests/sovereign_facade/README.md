...
- **Phase 1 (Playwright):** `services/gaiaos_ui_web/tests/sovereign_facade/` — spec and config live **inside** `gaiaos_ui_web` so `@playwright/test` resolves from that package (not `cells/fusion/node_modules`).
