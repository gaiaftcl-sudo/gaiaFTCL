...
export default async function AtmospherePage() {
  const [tiles, obs] = await Promise.all([
    fetchRecentTiles("atmosphere_tiles", 200, "valid_time"),
    fetchRecentObservations(100, "weather"),
  ]);

  return (
    <main className="mx-auto max-w-6xl px-4 py-6">
      <div className="mb-4">
        <div className="text-xl font-semibold text-white">Atmosphere</div>
        <div className="text-sm text-zinc-400">Tiles: atmosphere_tiles, Observations: weather</div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
          ...
          <div className="mt-3 space-y-2">
            {obs.observations.slice(0, 25).map((o: any) => (
