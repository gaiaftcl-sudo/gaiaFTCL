//! Train gaialm_med_core from Medical teacher harvests
//! HIGH-RISK: Virtue threshold 0.98
...
fn main() {
    ...
    println!("\n═══════════════════════════════════════════════════════════════");
    println!("🏥 GAIALM DISTILLATION: Medical → gaialm_med_core");
    ...
    let mut config = TrainConfig::medical();
    config.harvest_path = gaiaos_root.join("services/teacher_harvest/harvest_data/medical_harvest.json");
    ...
    println!("\n🏥 MEDICAL DISTILLATION COMPLETE\nNext: cargo run --bin gaialm_eval_medical\n");
}
